...
{
    public class GetAllInvoices(InvoiceHandler handler) : Endpoint<GetInvoicesRequest, List<InvoiceResponse>>
    {
        public override void Configure()
        {
            Post("api/v1/plecsys/invoice/all");
            AllowAnonymous();
        }

        public override async Task HandleAsync(GetInvoicesRequest request, CancellationToken cancellationToken)
        {
            try
            {
                var invoices = await handler.GetAllInvoices(request);
